...
var getUserAlbums = function (userId, callback, limit) {
  $.get('//jsonplaceholder.typicode.com/albums?userId=' + userId)
    .done(function (response) {
...
var getAlbumPhotos = function (albumId, callback, limit) {
  $.get('//jsonplaceholder.typicode.com/photos?albumId=' + albumId)
    .done(function (response) {
      callback(response.slice(0, limit))
    })
}
...
function getUsersPhotos(callback, limit) {
  ...
  getUsers(function(users) {
    ...
    users.forEach(function({id}){
      usersLeft++
      getUserAlbums(id, function(albums) {
        usersLeft--
        albums.forEach(function({id}) {
          albumsLeft++
          getAlbumPhotos(id, function(photos){
